...
import { Flow, FlowStep } from '../../domain/entities/Flow';
...
interface ExecutionContext {
  ...
  inputs: Record<string, unknown>; // Initial inputs provided to the flow
  outputs: Record<string, unknown>; // Final outputs generated by 'output' steps
}

/**
 * Service responsible for executing defined flows.
 * It manages the state of a flow execution, handles different step types (LLM, transform, output),
...
export class FlowExecutionService {
  ...
  /**
   * Executes a given flow with the provided inputs.
   * This is the main entry point for running a flow.
   *
   * Example:
   * const flow = //... get flow definition ...//;
   * const inputs = { query: "What is agentic AI?", userId: "user-123" };
   * const result = await flowExecutionService.executeFlow(flow, inputs);
   * // result might be: { executionId: '...', flowId: '...', success: true, outputs: { answer: 'Agentic AI is...' } }
   *
   ...
   */
  async executeFlow(flow: Flow, inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      this.validateInputs(flow, inputs);
      
      // Find the entry points (steps with no incoming connections)
      const entrySteps = this.findEntrySteps(flow);
      
      // Execute each entry step
      for (const step of entrySteps) {
        await this.executeStep(step, flow, context);
      }
  ...
  /**
   * Validates that all required inputs defined in the flow are present in the provided inputs.
   * Throws an error if a required input is missing and has no default value.
   *
   * Example:
   * // Flow defines input 'customerId' as required.
   * const inputs = { orderId: "order-456" };
   * // this.validateInputs(flow, inputs); // Throws Error("Required input 'customerId' is missing")
   *
   * @param flow - The flow definition containing input specifications.
   * @param inputs - The inputs provided for the execution.
   * @throws {Error} If a required input is missing.
   */
  private validateInputs(flow: Flow, inputs: Record<string, unknown>): void {
    for (const inputDef of flow.definition.inputs) {
  ...
  /**
   * Identifies the starting steps of the flow.
   * These are steps that do not have any other steps leading into them ('next' pointing to them).
   *
   * Example:
   * // Flow: StepA -> StepB, StepC -> StepB
   * // this.findEntrySteps(flow); // Returns [StepA, StepC]
   *
   * @param flow - The flow definition.
   * @returns An array of FlowStep objects that are entry points.
   */
  private findEntrySteps(flow: Flow): FlowStep[] {
    ...
    return flow.definition.steps.filter(step => !nextStepIds.has(step.id));
  }
  
  /**
   * Executes a single step within the flow, recursively calling itself for subsequent steps.
   ...
   * @param flow - The overall flow definition.
    ...
    try {
      ...
      if (step.next) {
        for (const nextStepId of step.next) {
          const nextStep = flow.definition.steps.find(s => s.id === nextStepId);
          if (nextStep) {
            await this.executeStep(nextStep, flow, context);
          } else {
   ...
   */
  private executeTransformStep(step: FlowStep, context: ExecutionContext): any {
   ...
   * @param context - The current execution context.
   */
  private executeOutputStep(step: FlowStep, context: ExecutionContext): void {
